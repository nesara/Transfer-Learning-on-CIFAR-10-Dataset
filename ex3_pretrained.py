...
class VggModel(nn.Module):
    def __init__(self, n_class, fine_tune, pretrained=True):
        ...
        #self.original_vgg11 = list(vgg11_bn.children())
        self.change_vgg11 = list(vgg11_bn.children())[0]
        #print("the list of change vgg11 :- ",self.change_vgg11)
        self.classifier = nn.Sequential(nn.Linear(layer_config[0], layer_config[1]), nn.BatchNorm1d(layer_config[1]), nn.ReLU(), nn.Linear(layer_config[1], num_classes))
...
if fine_tune:
    params_to_update = []
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    for param in model.change_vgg11.parameters():
        param.requires_grad = False
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        if current_accuracy > required_accuracy:
            required_accuracy = current_accuracy
            best_model = model
